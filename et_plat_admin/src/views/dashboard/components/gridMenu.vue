...
export default {
  ...
  data() {
    return {
      ...
      statisticData: [
        { title: '待发货订单', num: 0, path: '/order/list', perms: ['platform:order:page:list'] },
        { title: '退款中订单', num: 0, path: '/order/list', perms: ['platform:order:page:list'] },
        { title: '在售商品', num: 0, path: '/store/list', perms: ['platform:product:page:list'] },
        { title: '待审核商品', num: 0, path: '/store/list', perms: ['platform:product:page:list'] },
      ],
      optionData: {},
      applyNum: 0,
      style: { height: '250px' },
  ...
  methods: {
    ...
    getbusinessData() {
      businessData().then((res) => {
        ...
        this.statisticData[2].num = res.onSaleProductNum; //库存预警
        this.statisticData[3].num = res.awaitAuditProductNum; //上架商品
      });
    },
  },
};
