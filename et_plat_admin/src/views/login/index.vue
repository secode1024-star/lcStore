...
import { getLoginPicApi, captchaApi, codeCheckApi, captchaconfigApi } from '@/api/user';
...
export default {
  ...
  methods: {
    //获取币种
    getCurrency() {
      configGetUniq({ key: 'shop_pay_currency' }).then((res) => {
        let data = JSON.parse(res);
        localStorage.setItem('shopPayCurrency', data.symbol);
      });
    },
    handleResize(event) {
    ...
    handleLogin() {
      ...
      this.$refs.loginForm.validate((valid) => {
        if (valid) {
          this.captchatOn ? this.$refs.verify.show() : this.success(null);
        } else {
          return false;
        }
      });
    ...
    getCaptcha() {
      ...
      captchaApi()
        .then((data) => {
          ...
          console.log('🖼️ 设置验证码图片:', this.captchatImg ? '已设置' : '空白');
        })
        .catch((error) => {
