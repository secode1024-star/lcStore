  ...
  <div class="divBox">
    <el-card class="box-card">
      <div class="theme_box">
        ...
        <div class="save">
          <el-button type="primary" @click="saveTheme()" v-hasPermi="['platform:config:saveuniq']">保存</el-button>
        </div>
      </div>
    </el-card>
  </div>
...
<script>
import { themeSave, getTheme } from '@/api/systemGroup';
...
export default {
  ...
  methods: {
    ...
    getSet() {
      getTheme({ key: 'change_color_config' }).then((res) => {
        this.active = res - 1;
        if (res == 1) {
          this.$set(this, 'imgList', this.theme1);
        } else if (res == 2) {
