  ...
  <div class="divBox">
    ...
    <el-card class="box-card mt14" :body-style="{ padding: '0 20px 20px' }" shadow="never" :bordered="false">
      ...
      >
        ...
        <el-table-column label="内容标题" width="160" :show-overflow-tooltip="true">
          <template slot-scope="scope">
            <div class="line1">{{ scope.row.title | filterEmpty }}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="240" fixed="right">
          <template slot-scope="scope">
            <template
              ...
              "
            >
              <el-button type="text" @click="onAudit(scope.row.id, true)">审核</el-button>
            </template>
            <template v-if="checkPermi(['platform:community:note:repley:force:off:switch'])">
              <el-button type="text" @click="onReplyOff(scope.row)">{{
                scope.row.replyStatus !== 3 ? '关闭评论' : '取消关闭评论'
              }}</el-button>
            </template>
            <template v-if="scope.row.auditStatus == 1 && checkPermi(['platform:community:note:forced:down'])">
              <el-button type="text" @click="onOff(scope.row.id)">强制下架</el-button>
            </template>
      ...
      <div v-loading="loading" v-if="dialogVisible">
        <div class="detailHead">
          <div class="acea-row row-between headerBox">
            <div class="full">
              <div class="order_icon"><span class="iconfont iconmd-reverse-camera"></span></div>
              <div class="text">
