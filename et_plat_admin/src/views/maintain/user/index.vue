    ...
    <el-card class="box-card">
      <el-form ref="pram" :model="pram" :rules="rules" label-width="100px">
        ...
        <el-form-item>
          ...
          <el-button @click="close('pram')">重置</el-button>
        </el-form-item>
      </el-form>
    </el-card>
  </div>
...
export default {
  ...
  data() {
    ...
    const JavaPlatInfo = JSON.parse(Cookies.get('JavaPlatInfo'));
    return {
      password: '',
      JavaPlatInfo: JSON.parse(Cookies.get('JavaPlatInfo')),
      pram: {
        account: JavaPlatInfo.account,
        pwd: null,
        repwd: null,
        realName: JavaPlatInfo.realName,
        id: JavaPlatInfo.id,
      },
      roleList: [],
      rules: {
