    ...
    <el-card class="box-card" v-if="showForm">
      ...
      <el-form ref="form" :model="form" label-width="80px" :rules="rules">
        ...
        <el-form-item label="企业域名" prop="domain_name">
          <el-input v-model="form.domain_name" placeholder="请输入你部署的admin端的api域名"></el-input>
        </el-form-item>
...
<script>
import { authCertSubmit, authCertQuery } from '@/api/authInformation';
...
export default {
  ...
  data() {
    return {
      ...
      captchs: document.location.protocol + '//authorize.crmeb.net/api/captchs/',
      rules: {
        company_name: [{ required: true, message: '请填写您的企业名称', trigger: 'blur' }],
        domain_name: [{ required: true, message: '请输入域名，格式：baidu.com', trigger: 'blur' }],
        order_id: [{ required: true, message: '请输入您购买的源码订单号', trigger: 'blur' }],
  ...
  methods: {
    ...
    getAuthQuery() {
      configGetUniq({ key: 'authHost' }).then((data) => {
        this.authHost = data;
        authCertQuery({ host: data }).then((res) => {
