      ...
      <el-col v-bind="grid">
        <div class="Nav">
          ...
          <div class="trees-coadd">
            <div class="scollhide">
              <div class="trees" :style="{ maxHeight: !pictureType ? '300px' : '700px' }">
                <el-tree
                  ref="tree"
                  :data="treeData2"
                      ...
                      <el-dropdown>
                        ...
                        <el-dropdown-menu slot="dropdown">
                          ...
                          <el-dropdown-item
                            v-if="
                              node.label !== '全部图片' &&
                              checkPermi(['platform:category:update', 'platform:category:info'])
                            "
          ...
          <div class="bnt">
            ...
            >
              <el-button
                class="mr10"
                type="primary"
                v-if="pictureType && checkPermi(['platform:upload:image', 'platform:upload:file'])"
                >上传图片</el-button
              >
            </el-upload>
            <div>
              ...
              <el-button
                ...
                >批量下载 ({{ checkPicList.length }})</el-button
              >
            </div>
            <el-select
            ...
            <div class="conters scrollbarAll" :style="{ maxHeight: !pictureType ? '500px' : '700px' }">
              <div v-for="(item, index) in pictrueList.list" :key="index" class="gridPic">
                ...
                <img
                  ...
                  :src="item.sattDir ? processImageUrl(item.sattDir) : localImg"
                  :class="item.isSelect ? 'on' : ''"
                  @click="changImage(item, index, pictrueList.list)"
                  v-if="item.attType !== 'video/mp4'"
                />
                <video
                  :src="item.sattDir ? processImageUrl(item.sattDir) : ''"
                  :class="item.isSelect ? 'on' : ''"
                  @click="changImage(item, index, pictrueList.list)"
                  v-if="item.attType == 'video/mp4'"
                ></video>
...
export default {
  ...
  methods: {
    ...
    handleUploadForm(param) {
      if (!this.tableData.pid) this.tableData.pid = 0;
    ...
    editPicList(tit) {
      if (!this.checkPicList.length) return this.$message.warning('请先选择图片');
      this.$modalSure().then(() => {
        fileDeleteApi({ ids: this.ids.join(',') }).then(() => {
    ...
    async batchDownload() {
      if (!this.checkPicList.length) {
        return this.$message.warning('请先选择要下载的图片');
      }
      
      const loading = this.$loading({
        lock: true,
        text: `正在下载 ${this.checkPicList.length} 张图片，请稍候...`,
        spinner: 'el-icon-loading',
        background: 'rgba(0, 0, 0, 0.7)',
      });
      ...
      try {
        ...
        for (let i = 0; i < this.checkPicList.length; i++) {
          const item = this.checkPicList[i];
          try {
            await this.downloadSingleImage(item, i + 1);
            successCount++;
            
            // 添加延迟以避免过于频繁的下载
            if (i < this.checkPicList.length - 1) {
              await this.delay(300);
            }
          } catch (error) {
            console.error(`下载图片失败 (${i + 1}/${this.checkPicList.length}):`, error);
            failCount++;
          }
        }
        ...
        if (successCount > 0) {
          this.$message.success(`下载完成！成功：${successCount} 张，失败：${failCount} 张`);
        } else {
          this.$message.error('所有图片下载失败，请检查网络连接');
        }
        
        // 清空选择
        this.checkPicList = [];
        this.ids = [];
        this.getFileList(); // 刷新列表，取消选中状态
    ...
    // 下载单张图片
    async downloadSingleImage(item, index) {
      return new Promise((resolve, reject) => {
        try {
          const imageUrl = this.processImageUrl(item.sattDir);
          if (!imageUrl) {
          ...
          // 生成文件名
          const fileExtension = this.getFileExtension(imageUrl, item.attType);
          const fileName = `${item.name || `image_${index}`}.${fileExtension}`;
    ...
    // 获取文件扩展名
    getFileExtension(url, attType) {
      // 首先尝试从URL中提取扩展名
      const urlParts = url.split('.');
      if (urlParts.length > 1) {
        const ext = urlParts[urlParts.length - 1].split('?')[0]; // 移除查询参数
        if (/^(jpg|jpeg|png|gif|bmp|webp)$/i.test(ext)) {
      ...
      if (attType) {
        ...
        const ext = typeMap[attType.toLowerCase()];
        if (ext) return ext;
      }
