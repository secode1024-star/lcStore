...
-- 新增多语言字段
ADD COLUMN store_name_en VARCHAR(255) DEFAULT NULL COMMENT '商品名称-英文' AFTER store_name,
ADD COLUMN store_name_ru VARCHAR(255) DEFAULT NULL COMMENT '商品名称-俄语' AFTER store_name_en,
ADD COLUMN store_name_ar VARCHAR(255) DEFAULT NULL COMMENT '商品名称-阿拉伯语' AFTER store_name_ru,

ADD COLUMN store_info_en VARCHAR(500) DEFAULT NULL COMMENT '商品简介-英文' AFTER store_info,
ADD COLUMN store_info_ru VARCHAR(500) DEFAULT NULL COMMENT '商品简介-俄语' AFTER store_info_en,
ADD COLUMN store_info_ar VARCHAR(500) DEFAULT NULL COMMENT '商品简介-阿拉伯语' AFTER store_info_ru,

ADD COLUMN keyword_en VARCHAR(255) DEFAULT NULL COMMENT '关键字-英文' AFTER keyword,
ADD COLUMN keyword_ru VARCHAR(255) DEFAULT NULL COMMENT '关键字-俄语' AFTER keyword_en,
ADD COLUMN keyword_ar VARCHAR(255) DEFAULT NULL COMMENT '关键字-阿拉伯语' AFTER keyword_ru,

-- 新增详情图字段（从富文本中分离）
ADD COLUMN detail_images TEXT DEFAULT NULL COMMENT '详情图片URL列表，逗号分隔' AFTER slider_image,
ADD COLUMN detail_images_en TEXT DEFAULT NULL COMMENT '详情图片-英文' AFTER detail_images,
ADD COLUMN detail_images_ru TEXT DEFAULT NULL COMMENT '详情图片-俄语' AFTER detail_images_en,
ADD COLUMN detail_images_ar TEXT DEFAULT NULL COMMENT '详情图片-阿拉伯语' AFTER detail_images_ru,

-- 新增轮播图多语言字段
ADD COLUMN slider_image_en VARCHAR(2000) DEFAULT NULL COMMENT '轮播图-英文' AFTER slider_image,
ADD COLUMN slider_image_ru VARCHAR(2000) DEFAULT NULL COMMENT '轮播图-俄语' AFTER slider_image_en,
ADD COLUMN slider_image_ar VARCHAR(2000) DEFAULT NULL COMMENT '轮播图-阿拉伯语' AFTER slider_image_ru,

-- 新增价格字段（美元和人民币）
ADD COLUMN price_usd DECIMAL(10,2) DEFAULT 0.00 COMMENT '价格-美元' AFTER price,
ADD COLUMN ot_price_usd DECIMAL(10,2) DEFAULT 0.00 COMMENT '原价-美元' AFTER ot_price,

-- 新增审核状态字段
ADD COLUMN audit_status TINYINT(1) DEFAULT 0 COMMENT '审核状态：0-待审核，1-审核通过，2-审核拒绝' AFTER is_show,
...
-- 新增翻译状态字段
ADD COLUMN translation_status TINYINT(1) DEFAULT 0 COMMENT '翻译状态：0-未翻译，1-翻译中，2-翻译完成，3-翻译失败' AFTER audit_time,
ADD COLUMN translation_progress INT DEFAULT 0 COMMENT '翻译进度（0-100）' AFTER translation_status,
ADD COLUMN translation_error TEXT DEFAULT NULL COMMENT '翻译错误信息' AFTER translation_progress,
ADD COLUMN last_translation_time DATETIME DEFAULT NULL COMMENT '最后翻译时间' AFTER translation_error;

-- 修改字段注释
ALTER TABLE eb_store_product
MODIFY COLUMN store_name VARCHAR(255) NOT NULL COMMENT '商品名称-中文',
MODIFY COLUMN store_info VARCHAR(500) DEFAULT NULL COMMENT '商品简介-中文',
MODIFY COLUMN keyword VARCHAR(255) DEFAULT NULL COMMENT '关键字-中文',
MODIFY COLUMN slider_image VARCHAR(2000) DEFAULT NULL COMMENT '轮播图-中文',
MODIFY COLUMN price DECIMAL(10,2) NOT NULL DEFAULT 0.00 COMMENT '价格-人民币',
MODIFY COLUMN ot_price DECIMAL(10,2) DEFAULT 0.00 COMMENT '原价-人民币';
...
ADD COLUMN description_en TEXT DEFAULT NULL COMMENT '商品详情-英文' AFTER description,
ADD COLUMN description_ru TEXT DEFAULT NULL COMMENT '商品详情-俄语' AFTER description_en,
ADD COLUMN description_ar TEXT DEFAULT NULL COMMENT '商品详情-阿拉伯语' AFTER description_ru;
...
ADD COLUMN name_en VARCHAR(50) DEFAULT NULL COMMENT '分类名称-英文' AFTER name,
ADD COLUMN name_ru VARCHAR(50) DEFAULT NULL COMMENT '分类名称-俄语' AFTER name_en,
ADD COLUMN name_ar VARCHAR(50) DEFAULT NULL COMMENT '分类名称-阿拉伯语' AFTER name_ru,
ADD COLUMN translation_status TINYINT(1) DEFAULT 0 COMMENT '翻译状态：0-未翻译，1-翻译中，2-翻译完成，3-翻译失败' AFTER name_ar,
ADD COLUMN last_translation_time DATETIME DEFAULT NULL COMMENT '最后翻译时间' AFTER translation_status;

ALTER TABLE eb_product_category
MODIFY COLUMN name VARCHAR(50) NOT NULL COMMENT '分类名称-中文';
...
-- 新增询价相关字段
ADD COLUMN inquiry_no VARCHAR(32) NOT NULL COMMENT '询价单号' AFTER order_id,
ADD COLUMN country VARCHAR(100) DEFAULT NULL COMMENT '国家' AFTER user_address,
ADD COLUMN inquiry_status TINYINT(1) DEFAULT 0 COMMENT '询价状态：0-待处理，1-已处理，2-已废弃' AFTER status,
ADD COLUMN process_time DATETIME DEFAULT NULL COMMENT '处理时间' AFTER inquiry_status,
ADD COLUMN user_language VARCHAR(10) DEFAULT 'zh' COMMENT '用户提交时的语言：zh/en/ru/ar' AFTER process_time,

-- 新增联系方式翻译字段（保留原文）
ADD COLUMN real_name_original VARCHAR(50) DEFAULT NULL COMMENT '用户姓名-原文' AFTER real_name,
ADD COLUMN user_address_original VARCHAR(500) DEFAULT NULL COMMENT '用户地址-原文' AFTER user_address,
ADD COLUMN user_remark_original TEXT DEFAULT NULL COMMENT '用户备注-原文' AFTER mark;

-- 修改字段注释
ALTER TABLE eb_store_order
MODIFY COLUMN order_id VARCHAR(32) NOT NULL COMMENT '订单号（保留字段名，实际为询价单号）',
MODIFY COLUMN status TINYINT(1) NOT NULL DEFAULT 0 COMMENT '订单状态（保留字段名，实际为询价状态：0-待处理，1-已处理，2-已废弃）',
MODIFY COLUMN real_name VARCHAR(50) NOT NULL COMMENT '用户姓名-中文翻译',
MODIFY COLUMN user_address VARCHAR(500) NOT NULL COMMENT '用户地址-中文翻译',
MODIFY COLUMN mark TEXT DEFAULT NULL COMMENT '用户备注-中文翻译';
...
-- 新增商品名称多语言字段
ADD COLUMN product_name_en VARCHAR(255) DEFAULT NULL COMMENT '商品名称-英文' AFTER info,
ADD COLUMN product_name_ru VARCHAR(255) DEFAULT NULL COMMENT '商品名称-俄语' AFTER product_name_en,
ADD COLUMN product_name_ar VARCHAR(255) DEFAULT NULL COMMENT '商品名称-阿拉伯语' AFTER product_name_ru,

-- 新增用户看到的商品名称（用户提交时的语言）
ADD COLUMN product_name_user VARCHAR(255) DEFAULT NULL COMMENT '商品名称-用户看到的语言' AFTER product_name_ar,
ADD COLUMN user_language VARCHAR(10) DEFAULT 'zh' COMMENT '用户提交时的语言' AFTER product_name_user;
...
-- 例如：site_name -> site_name_en, site_name_ru, site_name_ar
...
-- 为常用系统配置添加多语言版本
-- 注意：需要先查询现有配置，确认哪些需要翻译，然后手动添加对应的多语言配置项
-- 常见需要翻译的配置：
-- - site_name (网站名称)
...
ADD COLUMN token_count INT DEFAULT 0 COMMENT 'Token消耗数量' AFTER translated_text,
ADD COLUMN cost DECIMAL(10,4) DEFAULT 0.0000 COMMENT '翻译成本（美元）' AFTER token_count,
ADD COLUMN model_name VARCHAR(50) DEFAULT NULL COMMENT '使用的模型名称' AFTER cost;
...
CREATE TABLE IF NOT EXISTS eb_product_image_translation (
    ...
    image_type VARCHAR(20) NOT NULL COMMENT '图片类型：slider-轮播图，detail-详情图',
    original_url VARCHAR(500) NOT NULL COMMENT '原始图片URL',
    image_index INT DEFAULT 0 COMMENT '图片索引（轮播图顺序）',

    zh_url VARCHAR(500) DEFAULT NULL COMMENT '中文图片URL',
    en_url VARCHAR(500) DEFAULT NULL COMMENT '英文图片URL',
    ru_url VARCHAR(500) DEFAULT NULL COMMENT '俄语图片URL',
    ar_url VARCHAR(500) DEFAULT NULL COMMENT '阿拉伯语图片URL',

    translation_status TINYINT(1) DEFAULT 0 COMMENT '翻译状态：0-未翻译，1-翻译中，2-翻译完成，3-翻译失败',
    error_message TEXT DEFAULT NULL COMMENT '错误信息',

    mer_id INT(11) NOT NULL COMMENT '商家ID',
    is_active TINYINT(1) DEFAULT 1 COMMENT '是否有效：1-有效，0-失效（商品重新上传图片后旧记录失效）',
...
ADD INDEX idx_category_audit (category_id, audit_status, is_show);
