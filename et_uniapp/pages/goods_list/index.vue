	...
	<view :data-theme="theme" class="merBox">
		<view class='productList'>
			...
			<view class='search bg_color acea-row row-between-wrapper'>
				<view class='input acea-row row-between-wrapper'><text class='iconfont icon-sousuo'></text>
					<input :placeholder='$t(`page.goodsList.search`)' placeholder-class='placeholder'
						confirm-type='search' name="search" v-model="where.keyword" @confirm="searchSubmit"
						maxlength="20"></input>
				</view>
			...
			<view class="nav-wrapper" v-if="merId===0 && where.cid === 0">
				<view class="tab-bar">
					<view class="tab-item" :class="{on:tabIndex===1}" @click="changetab(1)">{{$t(`page.goodsSearch.goods`)}}</view>
					<view class="tab-item" :class="{on:tabIndex===2}" @click="changetab(2)">{{$t(`page.goodsSearch.mer`)}}</view>
				</view>
			</view>
		
			<view  class='nav acea-row row-center-wrapper' :class="(merId > 0 || where.cid > 0) ? 'mer-nav' : ''">
			...
			<view class="" v-if="tabIndex===1" :class="is_switch==true?'proList':'listBox'">
				<view :class="[(merId > 0 || where.cid > 0) ? 'mer-listBox' : '']" v-if="productList.length>0">
					<view v-if="!is_switch" class='list acea-row row-between-wrapper' :class="[is_switch==true ? '' : 'on', (merId > 0 || where.cid > 0) ? 'mer-list' : ''] ">
						<view class='item' :class='is_switch==true?"":"on"' hover-class='none'
							...
							<view class='pictrue' :class='is_switch==true?"":"on"'>
								<easy-loadimage :image-src="item.image" :class='is_switch==true?"":"on"'></easy-loadimage>
							</view>
							<view class='text' :class='is_switch==true?"":"on"'>
								<view class='name line3'>{{item.storeName}}</view>
								<view class='vip acea-row row-between-wrapper' :class='is_switch==true?"":"on"'>
									<view class='x-money' :class='is_switch==true?"":"on"'>{{shopPayCurrency}}<text
				    ...
				    <view v-else class="goods">
				    	<WaterfallsFlow :shopPayCurrency="shopPayCurrency" :wfList='productList' :type="1" :isStore="1"/>
				    </view>
				</view>
		    ...
		    <view class="merList" v-if="tabIndex == 2">
		    	<merchant-list :shopPayCurrency="shopPayCurrency" :merchantList="merchantList"></merchant-list>
		    	<view class='loadingicon acea-row row-center-wrapper'>
		    		<text class='loading iconfont icon-jiazai' :hidden='loading==false'></text>
		    	</view>
		    </view>
		    <view class='noCommodity' :class="{'mer-mt194': tabIndex ==2, 'mer-mt100':  where.cid !== 0}" v-if="(productList.length==0 && isShow && where.page > 1 && tabIndex===1 && !loading )  || (merchantList.length==0 && isShow && where.page > 1 && tabIndex===2 && !loading) ">
		    	<view class='pictrue text-center'>
		    		<image src='../../static/images/noShopper.png'></image>
		    	</view>
				<text class="text-ccc">{{tabIndex===1?$t(`page.goodsList.empty`):$t(`page.goodsList.nomer`)}}</text>
		...
		methods: {
			...
			get_product_list: function(isPage) {
				...
				this.merId === 0 ? getProductslist(that.where).then(res => {
					...
					that.loadend = loadend;
					that.loading = false;
					that.loadTitle = loadend ? '已全部加载' : this.$t(`page.goodsList.more`);
