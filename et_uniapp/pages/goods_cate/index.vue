<template>
	<view class="page" :style="{height:winHeight + 'px'}">
		<view>
			<view class='productSort'>
				<tui-skeleton v-if="skeletonShow"></tui-skeleton>
				<view class="tui-skeleton">
					<uniNavBar background-color="#fff" color="#333" :border='false' @clickLeft='returns' @clickRight="open">
						<view class="header">
							<view class="input" @click="search()">
								<text class="iconfont icon-sousuo"></text>
								<text class="place">{{$t(`page.goodsDetail.search`)}}</text>
							</view>
						</view>
						<view slot="right" class="iconfont icon-more"></view>
					</uniNavBar>
					<view class='aside' :style="{bottom: tabbarH + 'px',height: height + 'rpx'}">
						<scroll-view scroll-y="true" scroll-with-animation='true' style="height: 100%;">
							<view class='item acea-row row-center-wrapper' :class='index==navActive?"on":""' v-for="(item,index) in categoryList"
						 :key="index" @click='onTap(index,"b"+index)'><text class="tui-skeleton-rect line1" style="padding-left: 10rpx;">{{item.name}}</text></view>
						 </scroll-view>
					</view>
					<view class='conter'>
						<scroll-view scroll-y="true" :scroll-into-view="toView" :style='"height:"+height+"rpx;"' @scroll="scroll"
						 scroll-with-animation='true'>
							<block v-for="(p,index) in categoryList" :key="index">
								<view class='listw' :id="'b'+index" v-for="(item,indexs) in p.childList" :key="indexs">
									<view class='title acea-row row-center-wrapper'>
										<view class='line'></view>
										<view class='name tui-skeleton-fillet'>{{item.name}}</view>
										<view class='line'></view>
									</view>
									<view class='list acea-row'>
										<block v-for="(itemn,indexn) in item.childList" :key="indexn">
											<navigator hover-class='none' :url='"/pages/goods_list/index?cid="+itemn.id+"&title="+itemn.name' class='item acea-row row-column row-middle'>
												<view class='picture tui-skeleton-circular' :style="{'background-color':itemn.icon?'none':'#f7f7f7'}">
													<image :src='itemn.icon'></image>
												</view>
												<view class='name line1'>{{itemn.name}}</view>
											</navigator>
										</block>
									</view>
								</view>
							</block>
							<view :style='"height:"+(height-300)+"rpx;"' v-if="number<15"></view>
						</scroll-view>
					</view>
				</view>
			</view>
			<tui-drawer mode="right" :visible="rightDrawer" @close="closeDrawer">
				<user-drawer @close="closeDrawer"></user-drawer>
			</tui-drawer>
		</view>
		<!-- 底部导航距离，做兼容处理的-->
		<view v-if="bottomNavigationIsCustom" class="footerBottom"></view>
		<!-- 底部导航 -->
		<pageFooter></pageFooter>
	</view>
</template>
<script>
	import pageFooter from "@/components/pageFooter/index.vue";
	import {mapGetters} from 'vuex';
	import {getCategoryList} from '@/api/store.js';
	const app = getApp();
	export default {
		data() {
			return {
				winHeight:'',
				configApi: {}, //分享类容配置
				navActive:0,
				
				skeletonShow: true, //骨架屏显示隐藏
				navlist: [],
				categoryList: [{name:'占位占位',child:[{extra:''},{extra:''}]},{name:'占位占位',child:[{extra:''},{extra:''}]},{name:'占位占位',child:[{extra:''},{extra:''}]},{name:'占位占位'}],				
				number: "",
				height: 0,
				hightArr: [],
				toView: "",
				tabbarH: 0,
				rightDrawer: false
			}
		},
		computed: mapGetters(['isLogin', 'uid', 'bottomNavigationIsCustom']),
		components:{
			pageFooter
		},
		onLoad(e){
			uni.setNavigationBarTitle({
				title: this.$t(`userDrawer.data[2].name`)
			})
			this.navActive = e.index?e.index:0;
			let that = this;
			uni.getSystemInfo({
			    success: function (res) {
			        that.winHeight = res.windowHeight;
			    }
			});
			this.getAllCategory();
			setTimeout(()=>this.onTap(this.navActive, 'b' + this.navActive), 1000);
		},
		methods: {
			open(){
				this.rightDrawer = true
			},
			closeDrawer(e) {
				this.rightDrawer = false
				if(!e){
					this.rightDrawer = false
				}
			},
			returns() {
				uni.navigateBack()
			},
			search(){
				uni.navigateTo({
					url:'/pages/goods_search/index'
				})
			},
			infoScroll: function() {
				let that = this;
				let len = that.categoryList.length;
				let child = that.categoryList[len - 1]&&that.categoryList[len - 1].childList?that.categoryList[len - 1].childList:[];
				this.number = child?child.length:0;
				
				//设置商品列表高度
				uni.getSystemInfo({
					success: function(res) {
						that.height = (res.windowHeight) * (750 / res.windowWidth) - 98;
					},
				});
				let height = 0;
				let hightArr = [];
				for (let i = 0; i < len; i++) {
					//获取元素所在位置
					let query = uni.createSelectorQuery().in(this);
					let idView = "#b" + i;
					query.select(idView).boundingClientRect();
					query.exec(function(res) {
						if(res){
							let top = res[0].top;
							hightArr.push(top);
							that.hightArr = hightArr
						}
					});
				};
			},
			//左侧点击一级分类
			onTap: function(index, id) {
				this.toView = id;
				this.navActive = index;
			},
			// 分类数据
			getAllCategory: function() {
				let that = this;
				getCategoryList().then(res => {
					that.categoryList = res.data;
					setTimeout(function(){
						that.infoScroll();
					},500)
					setTimeout(() => {
						this.skeletonShow = false
					}, 1000)
				})
			},
			scroll: function(e) {
				let scrollTop = e.detail.scrollTop;
				let scrollArr = this.hightArr;
				for (let i = 0; i < scrollArr.length; i++) {
					if (scrollTop >= 0 && scrollTop < scrollArr[1] - scrollArr[0]) {
						this.navActive = 0
					} else if (scrollTop >= scrollArr[i] - scrollArr[0] && scrollTop < scrollArr[i + 1] - scrollArr[0]) {
						this.navActive = i
					} else if (scrollTop >= scrollArr[scrollArr.length - 1] - scrollArr[0]) {
						this.navActive = scrollArr.length - 1
					}
				}
			},
			//搜索
			searchSubmitValue: function(e) {
				if (this.$util.trim(e.detail.value).length > 0)
					uni.navigateTo({
						url: '/pages/goods_list/index?searchValue=' + e.detail.value
					})
				else
					return this.$util.Tips({
						title: this.$t('page.goodsSearch.placeSearch')
					});
			},
		}
	}
</script>
<style lang="scss" scoped>
	.page{
		background: #fff;
		// overflow-y: auto;
	}
	.productSort .header .input {
		width: 558rpx;
		height: 60rpx;
		background-color: #f5f5f5;
		border-radius: 50rpx;
		box-sizing: border-box;
		padding: 0 30rpx;
		display: flex;
		align-items: center;
	}
	
	.productSort .header .input .iconfont {
		font-size: 26rpx;
		color: #666;
	}
	.productSort .header .input .place{
		padding-left: 12rpx;
		color: #ccc;
		font-size: 26rpx;
	}
	
	.productSort .aside {
		position: fixed;
		width: 180rpx;
		left: 0;
		top:0;
		background-color: #f7f7f7;
		overflow-y: scroll;
		overflow-x: hidden;
		
		height: auto;
		margin-top: 96rpx;
	}
	
	.productSort .aside .item {
		height: 100rpx;
		width: 100%;
		font-size: 26rpx;
		color: #424242;
		position: relative;
	}
	.productSort .aside .item.on {
		background-color: #fff;
		width: 100%;
		text-align: center;
		@include main_color(theme);
		font-weight: bold;
	}
	.productSort .aside .item.on ::before{
		content: '';
		width: 4rpx;
		height: 100rpx;
		position: absolute;
		left: 0;
		top: 0;
		@include main_bg_color(theme);
	}
	
	.productSort .conter {
		margin: 0 0 0 180rpx;
		padding: 0 14rpx;
		background-color: #fff;
	}
	
	.productSort .conter .listw {
		padding-top: 20rpx;
	}
	
	.productSort .conter .listw .title {
		height: 90rpx;
	}
	
	.productSort .conter .listw .title .line {
		width: 100rpx;
		height: 2rpx;
		background-color: #f0f0f0;
	}
	
	.productSort .conter .listw .title .name {
		font-size: 28rpx;
		color: #333;
		margin: 0 30rpx;
		font-weight: bold;
	}
	
	.productSort .conter .list {
		flex-wrap: wrap;
	}
	
	.productSort .conter .list .item {
		width: 177rpx;
		margin-top: 26rpx;
	}
	
	.productSort .conter .list .item .picture {
		width: 120rpx;
		height: 120rpx;
		border-radius: 50%;
	}
	
	.productSort .conter .list .item .picture image {
		width: 100%;
		height: 100%;
		border-radius: 50%;
		div{
			background-color: #f7f7f7;
		}
	}
	
	.productSort .conter .list .item .name {
		font-size: 24rpx;
		color: #333;
		height: 56rpx;
		line-height: 56rpx;
		width: 120rpx;
		text-align: center;
	}
</style>