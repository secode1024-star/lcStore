				...
				<view v-if="max > index">
					...
					<view v-if="list.auditStatus == 0 || list.auditStatus == 2 || list.auditStatus == 3"
						...
						<view v-if="list.auditStatus == 2 || list.auditStatus == 3" class="status-title">
							...
							<text class="title">{{list.auditStatus == 2 ? $t(`page.community.examineNo`) : $t(`page.community.examineClose`)}}，{{$t(`page.community.contentMy`)}}</text>
						</view>
						<view v-else class="status-title">
							<text class="iconfont icon-yingyeshijian2"></text>
							<text class="title">{{$t(`message.community.examineIng`)}}</text>
						</view>
						<view class="status_info">
							<text
								class="refusal">{{(list.auditStatus == 2 || list.auditStatus == 3) ? list.refusal : $t(`message.community.examineAfter`)}}</text>
						</view>
					</view>
					<view v-if="isShowCent" class="userInfo">
						...
						<view @click="likeToggle(list);" style="margin-top: 5px;" class="flex-column"
							...
							<text class="info-text">{{list.likeNum > 0 ? list.likeNum : $t(`page.community.like`)}}</text>
						</view>
						<!-- 3.评论 -->
						<view v-if="list.platReplySwitch" class="comment flex-column" @click="toComment(list)"
							...
							<text class="info-text">{{list.replyNum>0 ? list.replyNum : $t(`page.community.comment`)}}</text>
						</view>
						<!-- 4.分享 小程序端，审核通过之后-->
						<view v-if="list.auditStatus == 1" @click="listenerActionSheet" class="flex-column"
							style="margin-top: 17px;">
							<text class="iconfont icon-icon_transmit"></text>
							<text class="info-text">{{$t(`page.community.share`)}}</text>
						</view>
						<!-- 5.自己的视频 -->
							...
							<view class="video-my">
								...
								<view class="manage" v-show="showManage">
									...
									<view class="items" @click.stop="deleteTopic(list)">
										<text class="iconfont icon-caozuo-shanchu"></text>
										<text>{{$t(`page.community.delete`)}}</text>
									</view>
									<view disabled="true" class="items" @click.stop="replySwitch(list)">
										<text class="iconfont"
											:class="list.replyStatus ==1 ? 'icon-ic_daipingjia1' : 'icon-ic_daipingjia1'"></text>
										<text>{{list.replyStatus ==1 ? $t(`page.community.closeComment`) : $t(`page.community.openComment`)}}</text>
									</view>
								</view>
							</view>
						</view>
					...
					<view v-if="isShowCent" class="content">
						...
						<view class="words mb20" :style="'width: '+ (windowWidth - 120) +'px;'">
							<BaseTextMore v-if="list.content" :content="list.content" fontColor="rgba(255,255,255,0.9)"
								actionFontColor="#fff" :font-size="28" :rows="2" :expand-text="$t(`page.community.expand`)" :collapse-text="$t(`page.community.retract`)">
							</BaseTextMore>
									...
									<view class="item-count acea-row" @click="goDetail(goods)">
										...
										<view class="product-text">
											<view style="height: 68rpx;">
												...
												<view v-if="goods.isPay===1" class="buy">{{$t(`page.community.authorBought`)}}</view>
											</view>
											<view class="product-price">
												<view class="price">¥<text>{{goods.price}}</text></view>
												<view class="buy-btn">{{$t(`page.community.buy`)}}</view>
											</view>
										</view>
									</view>
								</view>
	...
	export default {
		...
		methods: {
			...
			async goPoster() {
				...
				let arr2
				uni.showLoading({
					title: this.$t(`message.community.covering`),
					mask: true
				});
				that.posters = false;
...
<style scoped lang="scss">
	...
	.icon-icon_transmit,.icon-icon_comment{
