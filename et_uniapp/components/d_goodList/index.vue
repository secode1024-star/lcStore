		...
		<view class="item" v-for="(item,index) in tempArr" :key='index' @click="goDetail(item)">
			...
			<view class="bottom acea-row row-between-wrapper">
				<view class="sales acea-row row-middle">
					<view class="money"><text>{{shopPayCurrency}}</text>{{item.price}} <text class="item_sales">{{$t(`page.goodsDetail.sales`)}} {{item.sales}}</text> </view>
				</view>
				<view v-if="item.stock>0">
				    <view class="bnt" v-if="item.activity && (item.activity.type === '1' || item.activity.type === '2' || item.activity.type === '3')">{{$t(`page.community.buyNow`)}}</view>
					<view v-else>
				...
				<view class="end" v-else>{{$t(`page.goodsDetail.soldOut`)}}</view>
			</view>
		</view>
	</view>
</template>
