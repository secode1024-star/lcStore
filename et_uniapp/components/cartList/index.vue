	...
	<view>
		<view class="cartList" :class="cartData.iScart?'on':''">
			<view class="title acea-row row-between-wrapper">
				<view class="name">{{$t(`page.goodsDetail.select`)}}</view>
				<view class="del acea-row row-middle" @click="subDel"><view class="iconfont icon-shanchu1"></view>{{$t(`page.goodsDetail.clear`)}}</view>
			</view>
			<view class="list">
				<view v-for="(itemn,index) in cartData.cartList" :key="index">
					<view class="item acea-row row-between-wrapper" v-for="(item,indexn) in itemn.cartInfoList" :key="indexn">
						<view class="pictrue">
							...
							<view v-if="item.attrStatus">
								...
								<view class="bottom acea-row row-between-wrapper">
									...
									<view class="cartNum acea-row row-middle">
										<view class="reduce iconfont icon-jian" @click="leaveCart(item, index)"></view>
										<view class="num">{{item.cartNum}}</view>
										<view class="plus iconfont icon-jia" @click="joinCart(item, index)"></view>
									</view>
								</view>
							</view>
...
<script>
	export default {
		...
		methods: {
			...
			leaveCart(item, index){
				this.$emit('ChangeCartNumDan', false, item, index);
			},
			joinCart(item, index){
				this.$emit('ChangeCartNumDan', true, item, index);
			},
			subDel(){
