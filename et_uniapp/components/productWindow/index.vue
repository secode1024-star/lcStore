	...
	<view>
		<view class="product-window"
				...
				<view class="text">
					...
					<view class="money">
						<view class="flex align-baseline">
							<text class="num mr10">{{shopPayCurrency}}{{ attr.productSelect.price }}</text>
							...
							<view>
								<text class="stock" v-if='isShow'>{{$t(`page.goodsDetail.inventory`)}}: {{ attr.productSelect.stock }}</text>
								<text class='stock' v-if="limitNum">{{$t(`page.goodsDetail.sales`)}}: {{attr.productSelect.quota}}</text>
							</view>
						</view>
			...
			<view class="rollTop">
				...
				<view class="cart acea-row row-between-wrapper">
					...
					<view class="carnum acea-row row-left">
						<view class="item reduce" :class="cart_num <= 1 ? 'on' : ''"
							@click="CartNumDes">
							-
						</view>
						<view class='item num'>
							<input type="number" :max="attr.productSelect.stock" v-model="cart_num"
								data-name="productSelect.cart_num"
								@input="bindCode(cart_num)"></input>
						</view>
						<view v-if="iSplus" class="item plus" :class="
				      cart_num >= attr.productSelect.stock
				        ? 'on'
				        : ''
				    " @click="CartNumAdd">
							+
						...
						<view v-else class='item plus'
							:class='(cart_num >= attr.productSelect.quota) || (cart_num >= attr.productSelect.stock) || (cart_num >= attr.productSelect.num)? "on":""'
			...
			<view class="joinBnt on"
				v-else-if="(iSbnt && attr.productSelect.quota<=0)||(iSbnt &&attr.productSelect.stock<=0)">{{$t(`page.goodsDetail.soldOut`)}}</view>
			<view class="joinBnt bg_color" v-if="iScart && attr.productSelect.stock" @click="goCat">{{$t(`page.goodsDetail.confirm`)}}</view>
			<view class="joinBnt on" v-else-if="(iScart && !attr.productSelect.stock)">{{$t(`page.goodsDetail.soldOut`)}}</view>
		</view>
	...
	export default {
		...
		watch: {
			attr: {
			      handler: function (val) {
			       this.cart_num = val.productSelect.cart_num
			      },
			      immediate: false,
			      deep: true,
			    }
		...
		data() {
			return {
				...
				cart_num: this.attr.productSelect.cart_num //购物车数量
			};
		},
		created() {
		...
		methods: {
			...
			bindCode: function(e) {
				let num = 0;
				if(e>=this.attr.productSelect.stock) {
					num=this.attr.productSelect.stock
				}else{
					num = e
				}
...
<style scoped lang="scss">
	...
	.product-window .textpic .text .line2 {
		height: 80rpx;
		line-height: 40rpx;
	}
