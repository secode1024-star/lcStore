  ...
  <div class="divBox">
    <el-card class="box-card mb20">
      <div slot="header" class="clearfix">
        <router-link :to="{ path: '/accounts/reconciliation' }">
          <el-button size="small" class="mr20 mb20" icon="el-icon-back">返回</el-button>
        </router-link>
      </div>
      <el-table
      ...
      >
          ...
          <template slot-scope="scope">
            <div v-for="(val, i) in scope.row.orderProduct" :key="i" class="tabBox acea-row row-middle">
              <div class="demo-image__preview">
                <el-image
                  :src="$imageUrl(val.cart_info.product.image)"
                  :preview-src-list="[$imageUrl(val.cart_info.product.image)]"
                />
              </div>
        ...
        <el-table-column :label="'商品总价 (' + GLOBAL.shopPayCurrency + ')'" min-width="150">
          <template slot-scope="scope">
            <span>{{ getTotal(scope.row.orderProduct) }}</span>
          </template>
        </el-table-column>
        <el-table-column :label="'佣金金额 (' + GLOBAL.shopPayCurrency + ')'" min-width="100">
          <template slot-scope="scope">
    ...
    <el-card class="box-card">
      ...
      >
        ...
        <el-table-column prop="refund_price" :label="'退款金额 (' + GLOBAL.shopPayCurrency + ')'" min-width="140" />
        <el-table-column prop="nickname" label="商品信息" min-width="330">
          <template slot-scope="scope">
            <div v-for="(val, i) in scope.row.refundProduct" :key="i" class="tabBox acea-row row-middle">
              <div class="demo-image__preview">
                <el-image
                  :src="$imageUrl(val.product.cart_info.product.image)"
              ...
              <span class="tabBox_pice">{{
                '￥' + val.product.cart_info.productAttr.price + ' x ' + val.product.product_num
              }}</span>
            </div>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      reconciliationOrderApi(this.$route.params.id, this.tableFrom)
        .then((res) => {
    ...
    getRefundList() {
      this.listLoading = true;
      reconciliationRefundApi(this.$route.params.id, this.tableFrom)
        .then((res) => {
