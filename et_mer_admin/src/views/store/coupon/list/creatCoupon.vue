    ...
    <el-card class="box-card">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="150px" class="demo-ruleForm">
        ...
        <el-form-item label="优惠券面值" prop="money">
          <el-input-number v-model="ruleForm.money" :min="1" :max="9999" label="描述文字"></el-input-number>
        </el-form-item>
        ...
        <el-form-item label="发布数量" prop="total" v-if="ruleForm.isLimited">
          <el-input-number v-model="ruleForm.total" :min="1" :max="9999" :step="1" step-strictly></el-input-number>
        </el-form-item>
        ...
        <el-form-item>
          <el-button
            ...
            :loading="loading"
            v-hasPermi="['merchant:coupon:save']"
            >立即创建</el-button
          >
...
<script>
import { couponSaveApi, couponInfoApi } from '@/api/store';
...
export default {
  ...
  methods: {
      ...
      couponInfoApi(this.$route.params.id)
        .then((res) => {
          ...
          this.ruleForm = {
            ...
            isForever: info.receiveEndTime ? true : false, //复制优惠券如果没有限制结束时间
            name: info.name,
            money: info.money,
    ...
    submitForm: Debounce(function (formName) {
      ...
      if ((this.ruleForm.isForever && !this.isForeverTime) || (this.ruleForm.isForever && !this.isForeverTime.length))
        return this.$message.warning('请选择请选择领取时间');
      if (this.ruleForm.useType === 2)
        this.ruleForm.primaryKey = this.ruleForm.checked
          .map((item) => {
            return item.id;
          })
          .join(',');
      ...
      this.ruleForm.isForever && this.isForeverTime.length
