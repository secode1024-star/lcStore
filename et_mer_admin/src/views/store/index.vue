    ...
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <el-tabs
          ...
          v-hasPermi="['merchant:product:tabs:headers']"
        >
          <el-tab-pane
        ...
        <div class="container mt-1">
          <el-form inline size="small">
            <el-form-item v-hasPermi="['merchant:product:category:cache:tree']" label="平台端商品分类：">
              <el-cascader
            ...
            <el-form-item v-hasPermi="['merchant:store:product:category:cache:tree']" label="商户商品分类：">
              <el-cascader
        ...
        <router-link :to="{ path: '/product/list/creatProduct' }">
      ...
      >
        ...
        <!--min-width="120"-->
        <!--align="center"-->
        <!--v-if="checkedCities.includes('操作时间')"-->
        <!--&gt;-->
        <!--<template slot-scope="scope">-->
        ...
        <el-table-column label="状态" min-width="80" fixed="right" v-if="checkedCities.includes('状态')">
          <template slot-scope="scope">
            <el-switch
              ...
              :disabled="Number(tableFrom.type) !== 1 && Number(tableFrom.type) !== 4 && Number(tableFrom.type) !== 2"
        ...
        <el-table-column label="操作" min-width="150" fixed="right" align="center" :render-header="renderHeader">
          <template slot-scope="scope">
            ...
            <router-link :to="{ path: '/product/list/creatProduct/' + scope.row.id }">
              <el-button
                type="text"
                size="small"
                class="mr10"
...
export default {
  ...
  computed: {
    ...mapGetters(['adminProductClassify', 'merProductClassify']),
  },
  activated() {
    this.goodHeade();
    this.getList();
  ...
  mounted() {
    ...
    this.checkedCities = this.$cache.local.has('goods_stroge')
      ...
      : this.checkedCities;
    if (checkPermi(['merchant:product:category:cache:tree'])) this.$store.dispatch('product/getAdminProductClassify');
