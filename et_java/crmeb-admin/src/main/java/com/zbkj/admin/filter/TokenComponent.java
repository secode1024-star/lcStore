...
//    private static final int expireTime = 30;
    ...
    public void delLoginUser(LoginUserVo loginUserVo) {
        if (ObjectUtil.isNotNull(loginUserVo)) {
            ...
            if (loginUserVo.getUser().getType().equals(SystemRoleConstants.SYSTEM_ROLE_TYPE_MERCHANT)) {
    ...
    public String createToken(LoginUserVo loginUser) {
        ...
        refreshToken(loginUser);
        if (loginUser.getUser().getType().equals(SystemRoleConstants.SYSTEM_ROLE_TYPE_PLATFORM) || loginUser.getUser().getType().equals(SystemRoleConstants.SYSTEM_ROLE_TYPE_PLATFORM_SUPER)) {
            token = TOKEN_PLATFORM_START_PREFIX + token;
        }
        if (loginUser.getUser().getType().equals(SystemRoleConstants.SYSTEM_ROLE_TYPE_MERCHANT) || loginUser.getUser().getType().equals(SystemRoleConstants.SYSTEM_ROLE_TYPE_MERCHANT_SUPER)) {
