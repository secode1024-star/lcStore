...
public class PlatformConfigController {
     ...
     */
    @PreAuthorize("hasAuthority('platform:config:info')")
     ...
     */
    @PreAuthorize("hasAuthority('platform:config:save:form')")
     ...
     * @param name name
     */
    @PreAuthorize("hasAuthority('platform:config:check')")
     ...
     * 配置表中仅仅存储对应的配置
     */
    @PreAuthorize("hasAuthority('platform:config:saveuniq')")
    @ApiOperation(value = "表单配置中仅仅存储")
    @RequestMapping(value = "/saveuniq", method = RequestMethod.POST)
    public CommonResult<Boolean> justSaveUniq(@RequestBody @Validated SystemConfigSaveRequest request) {
        return CommonResult.success(systemConfigService.updateOrSaveValueByName(request.getKey(), request.getValue()));
    }
     ...
     * @param key 配置表的的字段
     */
    @PreAuthorize("hasAuthority('platform:config:getuniq')")
     ...
     * 根据key获取配置
     */
    @PreAuthorize("hasAuthority('platform:config:get')")
