        ...
        if (requestURI.startsWith("/crmebimage/") || 
            ...
            if (httpResponse.getHeader("Access-Control-Allow-Origin") == null) {
                ...
                if (isAllowedOrigin(origin)) {
                    ...
                    httpResponse.setHeader("Access-Control-Allow-Credentials", "true"); // 允许凭证
                } else if (origin != null) {
                    // 如果有Origin但不在允许列表，仍然设置为请求的origin（图片资源宽松处理）
                    httpResponse.setHeader("Access-Control-Allow-Origin", origin);
            ...
            // 添加缓存控制（即使CORS头已存在，也可以设置缓存）
            if (httpResponse.getHeader("Cache-Control") == null) {
