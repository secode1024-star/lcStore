...
public class UploadServiceImpl implements UploadService {
    ...
    public FileResultVo imageUpload(MultipartFile multipartFile, String model, Integer pid, Integer owner) throws IOException {
        if (null == multipartFile || multipartFile.isEmpty()) {
            throw new CrmebException(MessageUtils.message("service.upload.error.a"));
        }
        ...
        if (StringUtils.isEmpty(extName)) {
            if (StrUtil.isNotBlank(multipartFile.getContentType())) {
                extName = multipartFile.getContentType().split("/")[1];
            } else {
                throw new CrmebException(MessageUtils.message("service.upload.error.b"));
            }
        }
        ...
        if( fileSize > uploadCommonVo.getSize()){
            throw new CrmebException(MessageUtils.message("service.upload.error.c", uploadCommonVo.getSize(), fs));
        }
        ...
        if (StringUtils.isNotEmpty(uploadCommonVo.getExtStr())) {
            ...
            if (extensionList.size() > 0) {
                ...
                if (!extensionList.contains(extName)) {
                    throw new CrmebException(MessageUtils.message("service.upload.error.d", uploadCommonVo.getExtStr()));
                }
            } else {
                throw new CrmebException(MessageUtils.message("service.upload.error.d", uploadCommonVo.getExtStr()));
            }
