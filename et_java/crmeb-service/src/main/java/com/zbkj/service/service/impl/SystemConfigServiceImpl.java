...
public class SystemConfigServiceImpl extends ServiceImpl<SystemConfigDao, SystemConfig> implements SystemConfigService {
    ...
    public String getValueByKeyException(String name) {
        ...
        if (null == value) {
            throw new CrmebException("not find "+ name +" data");
        }
    ...
    public Boolean updateOrSaveValueByName(String name, String value) {
        ...
        if (systemConfigs.size() >= 2) {
            throw new CrmebException(MessageUtils.message("service.systemConfig.error.a", name, systemConfigs.size()));
    ...
    public SystemConfig getColorConfig() {
        LambdaQueryWrapper<SystemConfig> lqw = Wrappers.lambdaQuery();
        lqw.eq(SystemConfig::getName, SysConfigConstants.CONFIG_CHANGE_COLOR_CONFIG);
     ...
     */
    @Override
    public String getMerchantSettlementAgreement() {
        LambdaQueryWrapper<SystemConfig> lqw = Wrappers.lambdaQuery();
        lqw.eq(SystemConfig::getName, SysConfigConstants.MERCHANT_SETTLEMENT_AGREEMENT);
        lqw.eq(SystemConfig::getStatus, 0);
    ...
    public String getUserRegisterAgreement() {
        LambdaQueryWrapper<SystemConfig> lqw = Wrappers.lambdaQuery();
        lqw.eq(SystemConfig::getName, SysConfigConstants.USER_REGISTER_AGREEMENT);
        lqw.eq(SystemConfig::getStatus, 0);
        SystemConfig systemConfig = dao.selectOne(lqw);
        if (ObjectUtil.isNull(systemConfig)) {
            return "";
    ...
    private void setRedisByVoList() {
        ...
        if (size > 0 || !crmebConfig.getAsyncConfig()) {
            return;
        }
