...
public class PageLayoutServiceImpl implements PageLayoutService {
    ...
    public HashMap<String, Object> index() {
        ...
        String indexBannerType = systemConfigService.getValueByKey(SysConfigConstants.FORNT_INDEX_BANNER_TYPE);
        map.put("indexBannerType", indexBannerType);
        // 首页金刚区
        ...
        // 首页新闻
        List<SystemGroupData> newsList = systemGroupDataService.findListByGid(GroupDataConstants.GROUP_DATA_ID_INDEX_NEWS_BANNER);
        ...
        // 店铺街、商品排行
        List<SystemGroupData> storeProductRankingList = systemGroupDataService.findListByGid(GroupDataConstants.GROUP_DATA_ID_STORE_PRODUCT_RANKING);
        map.put("indexRanking", convertData(storeProductRankingList));
        return map;
    }
    ...
    public Boolean save(JSONObject jsonObject) {
        ...
        if (StrUtil.isNotBlank(jsonObject.getString("indexNews"))) {
            ...
            dataList.addAll(convertGroupData(indexNews, GroupDataConstants.GROUP_DATA_ID_INDEX_NEWS_BANNER));
        }
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            systemGroupDataService.deleteByGid(GroupDataConstants.GROUP_DATA_ID_INDEX_NEWS_BANNER);
    ...
    public Boolean indexBannerSave(JSONObject jsonObject) {
        ...
        String indexBannerType = jsonObject.getString("indexBannerType");
        Boolean execute = transactionTemplate.execute(e -> {
            // 先删除历史数据
            systemGroupDataService.deleteByGid(GroupDataConstants.GROUP_DATA_ID_INDEX_BANNER);
            // 保存新数据
            systemGroupDataService.saveBatch(dataList, 100);
            systemConfigService.updateOrSaveValueByName(SysConfigConstants.FORNT_INDEX_BANNER_TYPE, indexBannerType);
            return Boolean.TRUE;
        });
        return execute;
    }
    ...
    public Boolean indexNewsSave(JSONObject jsonObject) {
        List<JSONObject> indexNews = CrmebUtil.jsonArrayToJsonObjectList(jsonObject.getJSONArray("indexNews"));
        List<SystemGroupData> dataList = convertGroupData(indexNews, GroupDataConstants.GROUP_DATA_ID_INDEX_NEWS_BANNER);
        Boolean execute = transactionTemplate.execute(e -> {
            // 先删除历史数据
            systemGroupDataService.deleteByGid(GroupDataConstants.GROUP_DATA_ID_INDEX_NEWS_BANNER);
            // 保存新数据
    ...
    public Boolean bottomNavigationSave(JSONObject jsonObject) {
        ...
        if (StrUtil.isBlank(isCustom)) {
            throw new CrmebException(MessageUtils.message("service.pageLayout.error.a"));
        }
        ...
        if (CollUtil.isEmpty(bottomNavigationList)) {
            throw new CrmebException(MessageUtils.message("service.pageLayout.error.b"));
        }
    ...
    public MyRecord getCategoryConfig() {
        String categoryConfig = systemConfigService.getValueByKey(SysConfigConstants.CONFIG_CATEGORY_CONFIG);// 商品分类页配置
        String isShowCategory = systemConfigService.getValueByKey(SysConfigConstants.CONFIG_IS_SHOW_CATEGORY);// 是否隐藏一级分类
    ...
    public Boolean categoryConfigSave(JSONObject jsonObject) {
        String categoryConfig = jsonObject.getString(SysConfigConstants.CONFIG_CATEGORY_CONFIG);
        String isShowCategory = jsonObject.getString(SysConfigConstants.CONFIG_IS_SHOW_CATEGORY);
        Boolean execute = transactionTemplate.execute(e -> {

            systemConfigService.updateOrSaveValueByName(SysConfigConstants.CONFIG_CATEGORY_CONFIG, categoryConfig);
            systemConfigService.updateOrSaveValueByName(SysConfigConstants.CONFIG_IS_SHOW_CATEGORY, isShowCategory);
            return Boolean.TRUE;
        });
        return execute;
    }
     ...
     */
    @Override
    public Boolean indexRankingSave(JSONObject jsonObject) {
        List<JSONObject> indexNews = CrmebUtil.jsonArrayToJsonObjectList(jsonObject.getJSONArray("indexRanking"));
        List<SystemGroupData> dataList = convertGroupData(indexNews, GroupDataConstants.GROUP_DATA_ID_STORE_PRODUCT_RANKING);
        return transactionTemplate.execute(e -> {
            // 先删除历史数据
            systemGroupDataService.deleteByGid(GroupDataConstants.GROUP_DATA_ID_STORE_PRODUCT_RANKING);
            // 保存新数据
            systemGroupDataService.saveBatch(dataList, 100);
            return Boolean.TRUE;
        });
    ...
    private List<SystemGroupData> convertGroupData(List<JSONObject> jsonObjectList, Integer gid) {
        return jsonObjectList.stream().map(e -> {
            ...
            e.forEach((key, value) -> {
                ...
                map.put("title", key);
                map.put("value", value);
                if (String.valueOf(value).contains(Constants.UPLOAD_TYPE_IMAGE)) {
