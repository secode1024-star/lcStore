...
public class CommunityCategoryServiceImpl extends ServiceImpl<CommunityCategoryDao, CommunityCategory> implements CommunityCategoryService {
    ...
    public void add(CommunityCategorySaveRequest request) {
        if (isExistName(request.getName(), 0)) {
            throw new CrmebException(CommunityResultCode.COMMUNITY_CATEGORY_EXIST.setMessage(MessageUtils.message("service.communityCategory.error.a")));
        }
        CommunityCategory category = new CommunityCategory();
        BeanUtils.copyProperties(request, category, "id");
        boolean save = save(category);
        if (!save) {
            throw new CrmebException(MessageUtils.message("service.communityCategory.error.b"));
        }
    }
    ...
    public void edit(CommunityCategorySaveRequest request) {
        if (ObjectUtil.isNull(request.getId())) {
            throw new CrmebException(CommunityResultCode.COMMUNITY_CATEGORY_ID_NULL.setMessage(MessageUtils.message("service.communityCategory.error.c")));
        }
        ...
        if (!update) {
            throw new CrmebException(MessageUtils.message("service.communityCategory.error.d"));
        }
    }
    ...
    public void deleteById(Integer id) {
        ...
        if (notesService.isUseCategory(id)) {
            throw new CrmebException(CommunityResultCode.COMMUNITY_CATEGORY_USING.setMessage(MessageUtils.message("service.communityCategory.error.e")));
        }
        category.setIsDel(Constants.COMMON_IS_FILED_ONE);
        boolean update = updateById(category);
        if (!update) {
            throw new CrmebException(CommonResultCode.ERROR.setMessage(MessageUtils.message("service.communityCategory.error.f")));
        }
    }
    ...
    public void showSwitch(Integer id) {
        ...
        boolean update = updateById(category);
        if (!update) {
            throw new CrmebException(MessageUtils.message("service.communityCategory.error.g"));
        }
    }
    ...
    private CommunityCategory getByIdExpetion(Integer id) {
        ...
        if (ObjectUtil.isNull(category) || category.getIsDel().equals(Constants.COMMON_IS_FILED_ONE)) {
            throw new CrmebException(CommunityResultCode.COMMUNITY_CATEGORY_NOT_EXIST.setMessage(MessageUtils.message("service.communityCategory.error.h")));
        }
        return category;
    }
}
