...
//    private static final int expireTime = 30;
    ...
    public String createToken(User user) {
        ...
        redisUtil.set(getTokenKey(token), user.getUid(), RedisConstants.TOKEN_EXPRESS_MINUTES, TimeUnit.MINUTES);
        return token;
    }
    ...
    public String getToken(HttpServletRequest request) {
        ...
        if (StrUtil.isNotEmpty(token) && token.startsWith(RedisConstants.USER_TOKEN_REDIS_KEY_PREFIX)) {
    ...
    public boolean checkRouter(String uri) {
        String[] routerList = {
                ...
                "api/front/login/google",
                "api/front/login/facebook",
                "api/front/login/email",
                "api/front/login/register/email",
                "api/front/login/email/captcha",
                "api/front/login/twitter/request/token",
                "api/front/login/twitter",
                "api/front/login/x-oauth"
        };
    ...
    public Boolean check(String token, HttpServletRequest request){
        ...
        try {
            ...
            if(exists){
                ...
                redisUtil.set(getTokenKey(token), uid, RedisConstants.TOKEN_EXPRESS_MINUTES, TimeUnit.MINUTES);
            }else{
