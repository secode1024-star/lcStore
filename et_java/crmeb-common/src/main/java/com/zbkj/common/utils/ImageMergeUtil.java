...
//            UploadUtil.setType(Constants.UPLOAD_TYPE_IMAGE);
    ...
    private static void drawImage(ImageMergeUtilVo imageMergeUtilVo){
        ...
        try{
            ...
            //合成图片xx失败
            throw new CrmebException(MessageUtils.message("common.imageMerge.error.a", fileName));
        }
    }
    ...
    private static void buildImage(List<ImageMergeUtilVo> list){
        if(list.size() < 2){
            throw new CrmebException(MessageUtils.message("common.imageMerge.error.b"));
        }
